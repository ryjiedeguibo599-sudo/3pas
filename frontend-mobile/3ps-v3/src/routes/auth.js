...
const safeUser = (u) => ({
  ...
  vehicle_type:  u.vehicle_type  ?? null,   // ← BAGO
...
router.post('/register', upload.single('profile_image'), async (req, res) => {
  try {
    const {
      full_name, email, phone, barangay,
      password, role, service_type, vehicle_type
    } = req.body
    ...
    if (phone && phone.trim() !== '' && !/^09\d{9}$/.test(phone.trim()))
    ...
    // Kung pasakay provider, kailangan ng vehicle_type
    if (role === 'provider' && service_type === 'pasakay') {
      if (!vehicle_type || !['motorcycle', 'tricycle', 'baobao'].includes(vehicle_type))
        return res.status(400).json({ message: 'Pumili ng uri ng sasakyan (Motorcycle, Tricycle, o Bao-bao).' })
    }

    // Check duplicate email
    const dupEmail = await db.query('SELECT id FROM users WHERE LOWER(email) = LOWER($1)', [email.trim()])
    ...
    const cleanVehicle   = (role === 'provider' && service_type === 'pasakay') ? vehicle_type : null

    const result = await db.query(
      `INSERT INTO users
        (full_name, email, phone, barangay, password, role, service_type, vehicle_type, profile_image)
...
// ── LOGIN ─────────────────────────────────────────────────────────────────────
// POST /api/auth/login
// Body: { login, password }  ← login = email OR phone
...
router.put('/profile', authMiddleware, upload.single('profile_image'), async (req, res) => {
  try {
    ...
    if (!fields.length) return res.status(400).json({ message: 'Walang pagbabago.' })
...
router.put('/change-password', authMiddleware, async (req, res) => {
  try {
    ...
    if (!isMatch) return res.status(401).json({ message: 'Mali ang kasalukuyang password.' })

    const isSame = await bcrypt.compare(new_password, user.password)
    if (isSame) return res.status(400).json({ message: 'Ang bago ay hindi dapat kapareho ng luma.' })
