          ...
          <ScrollView showsVerticalScrollIndicator={false} style={{ flex: 1 }}>
            ...
            <View style={modal.section}>
              ...
              {item.status === 'completed' && (
                isReviewed ? (
                  <View style={modal.ratedBadge}><Text style={modal.ratedText}>⭐  Na-rate mo na ito</Text></View>
                ) : (
              ...
              {item.status === 'cancelled' && (
                <TouchableOpacity style={modal.reorderBtn} onPress={() => { onClose(); navigation.navigate('PaRepair') }} activeOpacity={0.8}>
...
export default function MyRepairRequestsScreen({ navigation }) {
  ...
  const handleCancel = (requestId) => {
    Alert.alert('I-cancel ang Request?', 'Sigurado ka bang gusto mong i-cancel?', [
  ...
  return (
    <SafeAreaView style={styles.safe}>
        ...
        {filteredRequests.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptySubtitle}>
              {activeTab === 'Lahat' ? 'Mag-request ng repair at hahanapan ka namin ng repairman.' : `Wala kang ${activeTab.toLowerCase()} na repair requests.`}
            </Text>
            {activeTab === 'Lahat' && (
          ...
          filteredRequests.map((item) => {
            ...
            const goToWaiting = () => navigation.navigate('WaitingRepair', {
              description: item.description, address: item.address,
              repairType: item.repair_type, problemTitle: item.problem_title,
              schedule: item.schedule, requestId: item.id,
            })
            ...
            return (
              <View key={item.id} style={[styles.card, { borderLeftColor: sc, borderLeftWidth: 4 }]}>
                ...
                <View style={styles.previewBlock}>
                  {item.problem_title ? <Text style={styles.previewTitle} numberOfLines={1}>📋 {item.problem_title}</Text> : null}
                ...
                <View style={styles.actionRow}>
                  ...
                  {isPending && (
                    <>
                      <TouchableOpacity style={styles.btnTrack} onPress={goToWaiting} activeOpacity={0.85}>
                        <Text style={styles.btnTrackText}>🔍  I-track</Text>
                      </TouchableOpacity>
                      <TouchableOpacity
                      ...
                      >
                        {cancelling === item.id
                          ? <ActivityIndicator size="small" color={RED} />
                          : <Text style={styles.btnCancelText}>✕  I-cancel</Text>
                        }
                      </TouchableOpacity>
                    </>
                  )}
                  ...
                  {isOngoing && (
                    <TouchableOpacity style={[styles.btnTrack, { flex: 1 }]} onPress={goToWaiting} activeOpacity={0.85}>
                      <Text style={styles.btnTrackText}>🔧  Tingnan ang Status</Text>
                    </TouchableOpacity>
                  )}

                  {item.status === 'completed' && (
                    <TouchableOpacity style={[styles.btnTrack, { flex: 1 }]} onPress={() => openDetail(item)} activeOpacity={0.85}>
                      <Text style={styles.btnTrackText}>📋  Tingnan ang Detalye</Text>
                    </TouchableOpacity>
                  )}

                  {item.status === 'cancelled' && (
                    <TouchableOpacity style={[styles.btnTrack, { flex: 1, backgroundColor: '#64748b' }]} onPress={() => navigation.navigate('PaRepair')} activeOpacity={0.8}>
                      <Text style={styles.btnTrackText}>🔧  Mag-request Ulit</Text>
                    </TouchableOpacity>
                  )}
      ...
      <RatingModal
        ...
        serviceType="parepair" serviceId={ratingModal.requestId} onSuccess={fetchRequests}
      />
    </SafeAreaView>
  )
}
