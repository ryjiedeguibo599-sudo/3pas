...
export default function PadalaScreen({ navigation }) {
  ...
  const handleRequest = async () => {
    ...
    try {
      setLoading(true)
      await API.post('/padala/requests', {
        description,
        address,
        photo_urls: []
      })
      Alert.alert('Success', 'Na-send na ang delivery request mo!', [
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>📦 Padala</Text>
      </View>
      <Text style={styles.sectionTitle}>Ano ang ipapadala mo?</Text>
      <View style={styles.inputCard}>
        <Text style={styles.label}>Ilarawan ang padala</Text>
        <TextInput
          style={[styles.input, styles.textArea]}
          placeholder="ex: Isang kahon ng pagkain para sa lola sa Brgy. Campo"
          value={description}
      ...
      <View style={styles.inputCard}>
        <Text style={styles.label}>Address ng Tatanggap</Text>
        <TextInput
          style={styles.input}
      ...
      <View style={styles.inputCard}>
        <Text style={styles.label}>Uri ng Padala:</Text>
        <View style={styles.categoryRow}>
          {['Pagkain', 'Gamot', 'Dokumento', 'Damit', 'Others'].map(
            (cat) => (
      ...
      <View style={styles.infoCard}>
        ...
        <Text style={styles.infoText}>
          • Ang provider ay makikipag-ugnayan sa iyo sa loob ng 30 minuto
        </Text>
        <Text style={styles.infoText}>
          • Pwedeng bayaran ng cash o GCash
        </Text>
        <Text style={styles.infoText}>
          • May rating system para sa bawat provider
        </Text>
      </View>
      <TouchableOpacity
      ...
      >
        {loading
          ...
          : <Text style={styles.submitBtnText}>Mag-Request ng Padala!</Text>
        }
      </TouchableOpacity>
    </ScrollView>
  )
